...
namespace WPFMediaKit.DirectShow.Controls
{
    ...
    public class VideoCaptureElement : MediaElementBase
    {
        ...
        protected virtual void OnVideoCaptureSourceChanged(DependencyPropertyChangedEventArgs e)
        {
            ...
            {
                VideoCapturePlayerMySet = new VideoCapturePlayer();
                VideoCapturePlayerMySet.CameraControlSetter = new CameraControlSetter(VideoCaptureDevice);
                VideoCapturePlayerMySet.VideoProcAmpSetter = new VideoProcAmpSetter(VideoCaptureDevice);
        ...
        public void RefreshVideoCapture()
        {
            PlayerVideoCaptureSource();
        }
        ...
        {
            VideoCapturePlayer.UseYuv = (bool)e.NewValue;
        }
        ...
        public void SetBrightness(int brightnessVal)
        {
            VideoCapturePlayer.Dispatcher.BeginInvoke((Action)delegate
            {
                VideoCapturePlayer.VideoAProcAmp.Set(VideoProcAmpProperty.Brightness, brightnessVal, VideoProcAmpFlags.Manual);
            });
        }
        ...
        public void SetContrast(int iVal)
        {
            VideoCapturePlayer.Dispatcher.BeginInvoke((Action)delegate
            {
                VideoCapturePlayer.VideoAProcAmp.Set(VideoProcAmpProperty.Contrast, iVal, VideoProcAmpFlags.Manual);
            });
        }

        public void SetVideoProcAmpProperty(VideoProcAmpProperty videoProcAmpProperty, int iVal)
        {
            VideoCapturePlayer.Dispatcher.BeginInvoke((Action)delegate
            {
                int hr1 = VideoCapturePlayer.VideoAProcAmp.Set(videoProcAmpProperty, iVal, VideoProcAmpFlags.Manual);
            });
        }
        ...
        public void GetVideoProcAmpProperty(VideoProcAmpProperty videoProcAmpProperty)
        {
            VideoCapturePlayer.Dispatcher.BeginInvoke((Action)delegate
            {
                int hr1 = VideoCapturePlayer.VideoAProcAmp.Get(videoProcAmpProperty,out int iVal, out VideoProcAmpFlags videoProcAmpFlags);
            });
        }

        public List<Object> GetVideoProcAmpPropertyAuto(VideoProcAmpProperty videoProcAmpProperty)
        {
            List<object> listObj = new List<object>();
            VideoCapturePlayer.Dispatcher.BeginInvoke((Action)delegate
            {
                VideoProcAmpFlags videoProcAmpFlags = VideoProcAmpFlags.Auto;
                int hr1 = VideoCapturePlayer.VideoAProcAmp.Get(videoProcAmpProperty, out int iVal, out videoProcAmpFlags);
                listObj.Add(iVal);
                listObj.Add(videoProcAmpFlags);
            });
            return listObj;
        }
        ...
        protected VideoCapturePlayer VideoCapturePlayerMySet
        {
            get; set;
        }
        public int GetCameraControlParameterAsnyc(CameraControlProperty property, Action<int> callBackAction)
        {
            #region //这是依据星燊写过的设置亮度代码写的。针对VideoProcAmpProperty
            //VideoCapturePlayer.Dispatcher.BeginInvoke((Action)delegate
            //{
            //    VideoCapturePlayer.VideoAProcAmp.Get(VideoProcAmpProperty.Brightness, out int a
            //        , out VideoProcAmpFlags videoProcAmp);
            //});
            #endregion
            int iRet = VideoCapturePlayerMySet.CameraControlSetter.GetParameterValue(property);
            return iRet;
        }

        public CameraControlRangeParameter GetCameraControlRangeParameterAsync(CameraControlProperty property, Action<CameraControlRangeParameter> callBackAction)
        {
            CameraControlRangeParameter iRet = VideoCapturePlayerMySet.CameraControlSetter.GetRangeParameterValue(property);
            return iRet;
        }


        public int GetVideoProcAmpParameterAsnyc(VideoProcAmpProperty property, Action<int> callBackAction)
        {
            int iRet = VideoCapturePlayerMySet.VideoProcAmpSetter.GetParameterValue(property);
            return iRet;
        }
    }
